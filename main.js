...
var section = document.getElementsByClassName('section-title');
for (var i = 0; i < section.length; i++) {
  section[i].addEventListener("click", function() {
    var sectionContent = this.nextElementSibling;
    var sectionIcon = this.querySelector('img');

    if ( sectionContent.style.maxHeight ) {
      sectionContent.style.maxHeight = null;
      sectionIcon.src = './assets/images/icon-plus.svg';
    } else {
      sectionContent.style.maxHeight = sectionContent.scrollHeight + 'px';
      sectionIcon.src = './assets/images/icon-minus.svg';
    }
  });
}